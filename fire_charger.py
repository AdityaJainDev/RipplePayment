...
data = {"amount": sys.argv[1], "address": str(sys.argv[2])}
db.child("users").set(data)
print("The payment information is sent to Car")

while True:
	try:
		if (db.child("users").get().val()["conformation"] == True):
			print("\nThe payment conformation is successful")
